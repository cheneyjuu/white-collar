...
var _detail = _interopRequireDefault(__webpack_require__(/*! ./pages/race/detail.vue */ 54));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
/* harmony import */ var _detail_vue_vue_type_template_id_3a06d1e6___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./detail.vue?vue&type=template&id=3a06d1e6& */ 55);
...

{
  ...
  methods: {
    ...
    updateAccount: function updateAccount(payload) {
      ...
      var that = this;
      uni.request({
        url: "".concat(constants.baseUrl, "/global/account"),
        method: 'POST',
        data: payload,
        success: function success(res) {
          console.log(res);
    ...
    authentication: function authentication(openId) {
      ...
      uni.request({
        ...
        success: function success(res) {
          ...
          that.userInfo.avatarStyle = "background-image:url(".concat(that.userInfo.avatarUrl, ")");
    ...
    loadData: function loadData(raceId) {var _this2 = this;
      ...
      uni.request({
        ...
        success: function success(res) {var
          ...
          _this2.matchStartDate = data.matchStartDate;
          _this2.matchEndDate = data.matchEndDate;
          _this2.entryStartDate = data.entryStartDate;
          _this2.entryEndDate = data.entryEndDate;
          _this2.raceInfo = data;
        },
        fail: function fail() {},
        complete: function complete() {} });

    },
    loadItemData: function loadItemData(raceId) {var _this3 = this;
      var that = this;
      var token = uni.getStorageSync('id_token');
      var userInfo = uni.getStorageSync('userInfo');
      uni.request({
        url: "".concat(constants.baseUrl, "/races/").concat(raceId, "/items/").concat(userInfo.openId),
        method: 'GET',
        header: {
          'content-type': 'application/json',
          Authorization: 'Bearer ' + token },
    ...
    toRegister: function toRegister(item) {var
      ...
      uni.navigateTo({
        url: "/pages/race/register?raceId=".concat(raceId.id, "&itemId=").concat(itemId.id, "&offLineFlag=").concat(offLineFlag) });

    },
    startRace: function startRace(item) {
      var that = this;
      wx.getSetting({
        success: function success(res) {
          if (res.authSetting['scope.userLocation'] === false) {
            ...
            if (checkInType === 'IN_ORDER') {
              ...
              uni.navigateTo({
                url: "/pages/race/randomMap?lng=".concat(lng, "&lat=").concat(lat, "&raceId=").concat(raceId.id, "&itemId=").concat(itemId.id) });
