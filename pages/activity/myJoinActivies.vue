<template>
	<view>
		<cu-custom bgColor="bg-white" :isBack="true"><block slot="content">我的活动</block></cu-custom>
		<mescroll-uni ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption" :topbar="true" :top="70">
			<view class="cu-card case no-card" v-for="(item, index) in dataList" :key="index">
				<view class="cu-item shadow" @click="viewActivityDetails(item.activityId)">
...
export default {
	...
	methods: {
		...
		upCallback(page) {
			...
			uni.request({
				url: `${constants.baseUrl}/activities/login?page=${pageNum}&size=${pageSize}`,
				method: 'GET',
				header: {
